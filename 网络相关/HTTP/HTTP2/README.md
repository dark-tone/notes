...
### 缺点
Server Push 很难避免推送浏览器已经拥有的子资源，其实很多资源在浏览器第一次请求到就已经缓存下来了。这种 “过度推动” 会导致网络带宽的使用效率降低，从而显着阻碍性能优势。Chrome 数据显示 HTTP2/Push 实际上对整个网络的性能产生了负面影响。**Chrome已表明将在106版本后默认禁用http2 server push的功能。**
> 替代方案：使用early hints，即http的103状态码。
