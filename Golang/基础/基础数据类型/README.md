...
原生的字符串面值形式是`xxxx`，使用反引号代替双引号。在原生的字符串面值中，没有转义操作；全部的内容都是字面的意思，包含退格和换行，因此一个程序中的原生字符串面值可能跨越多行。
...
> Unicode定义了字符的唯一编码，而UTF-8则是Unicode的一种高效编码方式，用于将Unicode字符表示为字节序列。
